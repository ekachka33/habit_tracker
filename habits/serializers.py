...
class HabitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Habit
        fields = "__all__"
        # Пользователь устанавливается автоматически,
        # не должен быть доступен для записи
        read_only_fields = ("user", "last_notification_sent", "last_completed_at")
    ...
    def validate(self, data):
        ...
        # Валидация 1: Нельзя одновременно выбрать вознаграждение
        ...
        # Валидация 2: Время на выполнение привычки не должно превышать 120 с.
        if duration is not None and duration > 120:
            raise ValidationError(
                {
                    "duration": "Время на выполнение привычки не должно "
        ...
        if related_habit:
            ...
            # Это происходит при создании или обновлении,
            # когда related_habit - это int
            if isinstance(related_habit, int):
                try:
                    related_habit_obj = Habit.objects.get(pk=related_habit)
                except Habit.DoesNotExist:
                    raise ValidationError(
                        {"related_habit": "Связанная привычка не найдена."}
                    )
                # После получения объекта, используем его для дальнейшей
                # проверки
            ...
            if not related_habit.is_pleasant:
                raise ValidationError(
                    {
                        "related_habit": "В связанные привычки можно добавлять "
                        "только приятные привычки."
                    }
                )

        # Валидация 4: Приятная привычка не может иметь вознаграждения
        ...
        # Валидация 6: Для полезной привычки необходимо указать либо
    ...
    def create(self, validated_data):
        # Пользователь устанавливается автоматически
        # через perform_create в HabitViewSet
